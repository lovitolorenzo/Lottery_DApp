...
async function checkState() {
  ...
  const currentBlockDate = new Date(currentBlock.timestamp * 1000); // It takes milliseconds
  const closingTime = await contract.closingTime();
  console.log("closingTime: " + closingTime);
  console.log("closingTime: " + typeof closingTime.toNumber());
  const closingTimeDate = new Date(closingTime.toNumber() * 1000);
  // Useful time formatting methods :)
  // toLocaleTimeString() is meant to get the hour
  // toLocaleDateString() is meant to get date, like day/month/year
...
async function openBets(duration: string) {
  // Right way to get Current Time in blockchain instead of using Date.now()
  const currentBlock = await ethers.provider.getBlock("latest");
